/**
 * iconSearch.ts — pure, framework-free icon search.
 *
 * Extracted from useIconSearch so the ranking/matching algorithm can be unit
 * tested without React. The hook is now a thin useMemo wrapper around
 * `searchIcons`.
 *
 * Relies on each Icon carrying precomputed lowercase fields
 * (searchName / searchTags / searchCategory) so the hot loop never has to
 * re-lowercase on every keystroke.
 */
...
/**
 * Ranks `icons` against `query`. Returns a new array sorted by relevance.
 * An empty/whitespace query returns the input array unchanged (same reference).
 * Falls back to fuzzy "did you mean" suggestions (top 20) when nothing matches.
 */
